...
#include <vendor/sprd/hardware/wifi/supplicant/1.2/ISupplicantStaIfaceCallbackEx.h>
...
public:
	...
	int notifyOffloadRoam(struct wpa_supplicant *wpa_s, const char *reason);
#ifdef CONFIG_WAPI
	int notifyWapiDisconnectReason(struct wpa_supplicant *wpa_s, const char *reason);
#endif
	...
	int getStaIfaceHidlObjectByIfname(
	    const std::string &ifname,
	    android::sp<android::hardware::wifi::supplicant::V1_1::ISupplicantStaIface> *iface_object);
    ...
    int addStaIfaceCallbackHidlObjectEx(
        const std::string &ifname,
	...
	int addStaNetworkCallbackHidlObjectEx(
	    const std::string &ifname, int network_id,
...
private:
	...
	void removeStaIfaceCallbackHidlObject(
	    const std::string &ifname,
	    const android::sp<ISupplicantStaIfaceCallback> &callback);
	void removeStaIfaceCallbackHidlObjectEx(
	    const std::string &ifname,
	    const android::sp<ISupplicantStaIfaceCallbackEx> &callback);
	...
	void removeStaNetworkCallbackHidlObject(
	    const std::string &ifname, int network_id,
	    const android::sp<ISupplicantStaNetworkCallback> &callback);
	void removeStaNetworkCallbackHidlObjectEx(
		const std::string &ifname, int network_id,
		const android::sp<ISupplicantStaNetworkCallbackEx> &callback);
	...
	void callWithEachStaIfaceCallback_1_1(
	    const std::string &ifname,
	    const std::function<android::hardware::Return<void>(
		android::sp<android::hardware::wifi::supplicant::V1_1::ISupplicantStaIfaceCallback>)> &method);
	void callWithEachStaIfaceCallback_1_2(
	    const std::string &ifname,
	    const std::function<android::hardware::Return<void>(
	    android::sp<android::hardware::wifi::supplicant::V1_2::ISupplicantStaIfaceCallback>)> &method);
	void callWithEachStaIfaceCallback_sprd_1_2(
		const std::string &ifname,
		const std::function<android::hardware::Return<void>(
		android::sp<V1_2::ISupplicantStaIfaceCallbackEx>)> &method);
	...
	void callWithEachStaNetworkCallbackEx(
	    const std::string &ifname, int network_id,
	    const std::function<android::hardware::Return<void>(
		android::sp<ISupplicantStaNetworkCallbackEx>)> &method);
	...
	// Map of all the callbacks registered for Sprd STA interface specific
	// hidl objects controlled by wpa_supplicant. This map is keyed in by
	// the corresponding |ifname|.
	std::map<
	    const std::string,
		std::vector<android::sp<ISupplicantStaIfaceCallbackEx>>>
		sta_iface_callbacks_map_ex_;
	...
	// Map of all the callbacks registered for Sprd STA network specific
	// hidl objects controlled by wpa_supplicant.  This map is keyed in by
	// the corresponding |ifname| & |network_id|.
	std::map<
	    const std::string,
...
static_assert(
    static_cast<uint32_t>(android::hardware::wifi::supplicant::V1_0::ISupplicant::DebugLevel::EXCESSIVE) == MSG_EXCESSIVE,
    "Debug level value mismatch");
static_assert(
    static_cast<uint32_t>(android::hardware::wifi::supplicant::V1_0::ISupplicant::DebugLevel::ERROR) == MSG_ERROR,
    "Debug level value mismatch");
