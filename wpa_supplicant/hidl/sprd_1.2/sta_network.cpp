...
using vendor::sprd::hardware::wifi::supplicant::V1_2::ISupplicantStaNetwork;
...
constexpr uint32_t kAllowedKeyMgmtMask =
    (static_cast<uint32_t>(ISupplicantStaNetwork::KeyMgmtMask::NONE) |
     ...
     static_cast<uint32_t>(ISupplicantStaNetwork::KeyMgmtMaskEx::WAPI_PSK) |
     static_cast<uint32_t>(ISupplicantStaNetwork::KeyMgmtMaskEx::WAPI_CERT));
constexpr uint32_t kAllowedProtoMask =
    (static_cast<uint32_t>(ISupplicantStaNetwork::ProtoMask::WPA) |
     ...
     static_cast<uint32_t>(ISupplicantStaNetwork::ProtoMaskEx::WAPI));
...
Return<void> StaNetwork::registerCallback(
    const sp<android::hardware::wifi::supplicant::V1_0::ISupplicantStaNetworkCallback> &callback,
...
Return<void> StaNetwork::registerCallback_sprd_1_2(
    const sp<ISupplicantStaNetworkCallbackEx> &callback,
	registerCallback_sprd_1_2_cb _hidl_cb)
{
	return validateAndCall(
	    this, SupplicantStatusCode::FAILURE_NETWORK_INVALID,
	    &StaNetwork::registerCallbackInternal_sprd, _hidl_cb, callback);
}
...
SupplicantStatus StaNetwork::registerCallbackInternal_sprd(
    const sp<ISupplicantStaNetworkCallbackEx> &callback)
{
	HidlManager *hidl_manager = HidlManager::getInstance();
	if (!hidl_manager || hidl_manager->addStaNetworkCallbackHidlObjectEx(
				 ifname_, network_id_, callback)) {
		return {SupplicantStatusCode::FAILURE_UNKNOWN, ""};
}
	return {SupplicantStatusCode::SUCCESS, ""};
...
Return<void> StaNetwork::setNetworkParam(const hidl_string& key, const hidl_string& value, setNetworkParam_cb _hidl_cb) {
    return validateAndCall(
        this, SupplicantStatusCode::FAILURE_NETWORK_INVALID,
        &StaNetwork::setNetworkParamInternal, _hidl_cb, key, value);
}

Return<void> StaNetwork::getNetworkParam(const hidl_string& key, getNetworkParam_cb _hidl_cb) {
    return validateAndCall(
        this, SupplicantStatusCode::FAILURE_NETWORK_INVALID,
        &StaNetwork::getNetworkParamInternal, _hidl_cb, key);
}

SupplicantStatus StaNetwork::setNetworkParamInternal(const std::string& key, const std::string& value)
{
    struct wpa_ssid *wpa_ssid = retrieveNetworkPtr();
    wpa_printf(MSG_ERROR, "setNetworkParamInternal key=%s, value=%s", key.c_str(), value.c_str());
    //Example(Do not remove):set as the following if the key.c_str() is "bgscan".
    //if (setStringFieldAndResetState(
    //    value.c_str(), &(wpa_ssid->bgscan), "bgscan")) {
    //    return {SupplicantStatusCode::FAILURE_UNKNOWN, ""};
    //}
#ifdef CONFIG_WAPI
    if (!key.compare("psk_key_type")) {
        wpa_ssid->psk_key_type = atoi(value.c_str());
        resetInternalStateAfterParamsUpdate();
    } else if (!key.compare("wapi_as_cert")) {
        wpa_printf(MSG_INFO, "wapi as cert settings: %p", &wpa_ssid->wapi_as_cert);
        if (setStringFieldAndResetState(value.c_str(), &(wpa_ssid->wapi_as_cert), "wapi_as_cert")) {
            return {SupplicantStatusCode::FAILURE_UNKNOWN, ""};
        }
    } else if (!key.compare("wapi_user_cert")) {
        wpa_printf(MSG_INFO, "wapi user cert settings: %p", &wpa_ssid->wapi_user_cert);
        if (setStringFieldAndResetState(value.c_str(), &(wpa_ssid->wapi_user_cert), "wapi_user_cert")) {
            return {SupplicantStatusCode::FAILURE_UNKNOWN, ""};
        }
    } else
#endif
    if (!key.compare("sim_num")) {
        wpa_ssid->eap.sim_num = atoi(value.c_str());
        resetInternalStateAfterParamsUpdate();
    }
    return {SupplicantStatusCode::SUCCESS, ""};
}

std::pair<SupplicantStatus, std::string>StaNetwork::getNetworkParamInternal(const std::string& key)
{
    struct wpa_ssid *wpa_ssid = retrieveNetworkPtr();
    wpa_printf(MSG_ERROR, "getNetworkParam key=%s", key.c_str());
    if (!key.compare("sim_num")) {
        std::stringstream tempSs;
        std::string str;
        tempSs << wpa_ssid->eap.sim_num;
        tempSs >> str;
        return {{SupplicantStatusCode::SUCCESS, ""}, str};
    }
    return {{SupplicantStatusCode::SUCCESS, ""},""};
}
