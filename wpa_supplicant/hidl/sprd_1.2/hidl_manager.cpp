...
int HidlManager::registerInterface(struct wpa_supplicant *wpa_s)
{
	...
	if (isP2pIface(wpa_s)) {
		...
		// here!
		sta_iface_callbacks_map_ex_[wpa_s->ifname] =
		    std::vector<android::sp<ISupplicantStaIfaceCallbackEx>>();
		//
...
int HidlManager::unregisterInterface(struct wpa_supplicant *wpa_s)
{
	...
	if (success) {  // assumed to be P2P
		...
		if (success) {
			success = !removeAllIfaceCallbackHidlObjectsFromMap(
				wpa_s->ifname, sta_iface_callbacks_map_ex_);
		}
	}
	if (!success) {
		wpa_printf(
    ...
    struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid)
{
	...
	if (isP2pIface(wpa_s)) {
		...
		sta_network_callbacks_map_ex_[network_key] =
		    std::vector<android::sp<ISupplicantStaNetworkCallbackEx>>();
		// Invoke the |onNetworkAdded| method on all registered
		// callbacks.
    ...
    struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid)
{
	...
	if (isP2pIface(wpa_s)) {
		...
		if (removeAllNetworkCallbackHidlObjectsFromMap(
			...
			return 1;
		if (removeAllNetworkCallbackHidlObjectsFromMap(
			network_key, sta_network_callbacks_map_ex_))
			return 1;
		// Invoke the |onNetworkRemoved| method on all registered
		// callbacks.
		callWithEachStaIfaceCallback(
...
/**
 * Notify all listeners about any offload roam event.
 *
 * @param wpa_s |wpa_supplicant| struct corresponding to the interface on which
 * the network is present.
 * @param reason reason of offload roam.
 */
int HidlManager::notifyOffloadRoam(struct wpa_supplicant *wpa_s, const char *reason)
{
	// Invoke the |onEvent | method on all registered callbacks
	callWithEachStaIfaceCallback_sprd_1_2(
		wpa_s->ifname, std::bind(
					&ISupplicantStaIfaceCallbackEx::onEventEx,
					std::placeholders::_1,
					reason));
	return 0;
}
...
#ifdef CONFIG_WAPI
/**
 * Notify all listeners about wapi disconnect event
 * 
 * @param wpa_s |wpa_supplicant| struct corresponding to the interface on which
 * the network is present.
 * @param reason reason of wapi disconnect
 */
int HidlManager::notifyWapiDisconnectReason(struct wpa_supplicant *wpa_s, const char *reason){
    // Invoke the |onEvent| method on all registered
    // callbacks.
    callWithEachStaIfaceCallback_sprd_1_2(
        wpa_s->ifname, std::bind(
					&ISupplicantStaIfaceCallbackEx::onEventEx,
...
{
	typedef android::hardware::wifi::supplicant::V1_1::ISupplicantStaIfaceCallback::EapErrorCode EapErrorCode;
	...
	callWithEachStaIfaceCallback_1_1(
	    wpa_s->ifname,
	    std::bind(
		&android::hardware::wifi::supplicant::V1_1::ISupplicantStaIfaceCallback::onEapFailure_1_1,
		...
		struct wpa_ssid *config)
{
	...
	callWithEachStaIfaceCallback_1_2(hidl_ifname,
			std::bind(
					&android::hardware::wifi::supplicant::V1_2::ISupplicantStaIfaceCallback::onDppSuccessConfigReceived,
...
void HidlManager::notifyDppConfigSent(struct wpa_supplicant *wpa_s)
{
	...
	callWithEachStaIfaceCallback_1_2(hidl_ifname,
			std::bind(&android::hardware::wifi::supplicant::V1_2::ISupplicantStaIfaceCallback::onDppSuccessConfigSent,
					std::placeholders::_1));
}
...
void HidlManager::notifyDppFailure(struct wpa_supplicant *wpa_s, DppFailureCode code)
{
	...
	callWithEachStaIfaceCallback_1_2(hidl_ifname,
			std::bind(&android::hardware::wifi::supplicant::V1_2::ISupplicantStaIfaceCallback::onDppFailure,
...
void HidlManager::notifyDppProgress(struct wpa_supplicant *wpa_s, DppProgressCode code)
{
	...
	callWithEachStaIfaceCallback_1_2(hidl_ifname,
			std::bind(&android::hardware::wifi::supplicant::V1_2::ISupplicantStaIfaceCallback::onDppProgress,
					std::placeholders::_1, code));
}
 ...
 * @return 0 on success, 1 on failure.
 */
int HidlManager::addStaIfaceCallbackHidlObjectEx(
    const std::string &ifname,
    const android::sp<ISupplicantStaIfaceCallbackEx> &callback)
{
	const std::function<void(
	    const android::sp<ISupplicantStaIfaceCallbackEx> &)>
	    on_hidl_died_fctor = std::bind(
		&HidlManager::removeStaIfaceCallbackHidlObjectEx, this, ifname,
		std::placeholders::_1);
	return addIfaceCallbackHidlObjectToMap(
	    ifname, callback, on_hidl_died_fctor, sta_iface_callbacks_map_ex_);
}
 ...
 * @return 0 on success, 1 on failure.
 */
int HidlManager::addStaNetworkCallbackHidlObjectEx(
    const std::string &ifname, int network_id,
    const android::sp<ISupplicantStaNetworkCallbackEx> &callback)
{
	const std::function<void(
	    const android::sp<ISupplicantStaNetworkCallbackEx> &)>
	    on_hidl_died_fctor = std::bind(
		&HidlManager::removeStaNetworkCallbackHidlObjectEx, this, ifname,
		network_id, std::placeholders::_1);
	return addNetworkCallbackHidlObjectToMap(
	    ifname, network_id, callback, on_hidl_died_fctor,
	    sta_network_callbacks_map_ex_);
}
 ...
 * @param callback Hidl reference of the callback object.
 */
void HidlManager::removeStaIfaceCallbackHidlObjectEx(
    const std::string &ifname,
    const android::sp<ISupplicantStaIfaceCallbackEx> &callback)
{
	return removeIfaceCallbackHidlObjectFromMap(
	    ifname, callback, sta_iface_callbacks_map_ex_);
}
 ...
 * @param callback Hidl reference of the callback object.
 */
void HidlManager::removeStaNetworkCallbackHidlObjectEx(
    const std::string &ifname, int network_id,
    const android::sp<ISupplicantStaNetworkCallbackEx> &callback)
{
	return removeNetworkCallbackHidlObjectFromMap(
	    ifname, network_id, callback, sta_network_callbacks_map_ex_);
}
...
void HidlManager::callWithEachStaIfaceCallback_1_1(
    const std::string &ifname,
    const std::function<
	Return<void>(android::sp<android::hardware::wifi::supplicant::V1_1::ISupplicantStaIfaceCallback>)> &method)
{
...
void HidlManager::callWithEachStaIfaceCallback_1_2(
    const std::string &ifname,
    const std::function<
	Return<void>(android::sp<android::hardware::wifi::supplicant::V1_2::ISupplicantStaIfaceCallback>)> &method)
{
 ...
 * @param ifname Name of the corresponding interface.
 * @param method Pointer to the required hidl method from
 * |ISupplicantIfaceCallbackEx|.
 */
void HidlManager::callWithEachStaIfaceCallback_sprd_1_2(
	const std::string &ifname,
	const std::function<
	Return<void>(android::sp<V1_2::ISupplicantStaIfaceCallbackEx>)> &method)
{
	callWithEachIfaceCallback(ifname, method, sta_iface_callbacks_map_ex_);
}
 ...
 * @param network_id ID of the corresponding network.
 * @param method Pointer to the required hidl method from
 * |ISupplicantP2pNetworkCallback| or |ISupplicantStaNetworkCallbackEx| .
 */
void HidlManager::callWithEachStaNetworkCallbackEx(
    const std::string &ifname, int network_id,
    const std::function<
	Return<void>(android::sp<ISupplicantStaNetworkCallbackEx>)> &method)
{
	callWithEachNetworkCallback(
	    ifname, network_id, method, sta_network_callbacks_map_ex_);
}
