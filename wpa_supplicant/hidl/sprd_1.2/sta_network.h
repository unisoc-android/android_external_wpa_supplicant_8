...
#include <vendor/sprd/hardware/wifi/supplicant/1.2/ISupplicantStaNetwork.h>
#include <vendor/sprd/hardware/wifi/supplicant/1.2/ISupplicantStaNetworkCallbackEx.h>
...
public:
	...
	Return<void> setNetworkParam(const hidl_string& key,
	    const hidl_string& value, setNetworkParam_cb _hidl_cb) override;
	Return<void> getNetworkParam(const hidl_string& key,
	    getNetworkParam_cb _hidl_cb) override;
...
private:
	...
	SupplicantStatus setNetworkParamInternal(const std::string& key, const std::string& value);
	std::pair<SupplicantStatus, std::string> getNetworkParamInternal(const std::string& key);
	SupplicantStatus registerCallbackInternal_sprd(
	    const sp<ISupplicantStaNetworkCallbackEx> &callback);
