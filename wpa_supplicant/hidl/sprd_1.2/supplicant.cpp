...
Supplicant::addInterfaceInternal(const IfaceInfo& iface_info)
{
	...
	if (iface_info.type == IfaceType::P2P) {
		...
		if (ensureConfigFileExists(
			...
			wpa_global_->params.conf_p2p_dev = strdup(kP2pIfaceConfPath);
		}
...
#ifdef CONFIG_WAPI
	if (wpa_supplicant_get_iface(wpa_global_, "wlan0") &&
	    wpa_supplicant_get_iface(wpa_global_, "wlan0")->wapi)
		wpa_supplicant_get_iface(wpa_global_, "wlan0")->wapi
			->iwn_set_debug_level(static_cast<uint32_t>(level));
#endif
	return {SupplicantStatusCode::SUCCESS, ""};
}
