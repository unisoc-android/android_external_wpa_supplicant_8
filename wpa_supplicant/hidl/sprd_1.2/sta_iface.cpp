...
using vendor::sprd::hardware::wifi::supplicant::V1_2::ISupplicantStaIface;
using vendor::sprd::hardware::wifi::supplicant::V1_2::implementation::HidlManager;
...
Return<void> StaIface::registerCallback_1_1(
    const sp<android::hardware::wifi::supplicant::V1_1::ISupplicantStaIfaceCallback> &callback,
    registerCallback_cb _hidl_cb)
{
	sp<android::hardware::wifi::supplicant::V1_0::ISupplicantStaIfaceCallback> callback_1_0 = callback;
	return validateAndCall(
	    this, SupplicantStatusCode::FAILURE_IFACE_INVALID,
...
Return<void> StaIface::registerCallback_1_2(
    const sp<android::hardware::wifi::supplicant::V1_2::ISupplicantStaIfaceCallback> &callback,
    registerCallback_cb _hidl_cb)
{
	sp<android::hardware::wifi::supplicant::V1_1::ISupplicantStaIfaceCallback> callback_1_1 = callback;
	return validateAndCall(
	    this, SupplicantStatusCode::FAILURE_IFACE_INVALID,
	    &StaIface::registerCallbackInternal, _hidl_cb, callback_1_1);
}

Return<void> StaIface::registerCallback_sprd_1_2(
    const sp<ISupplicantStaIfaceCallbackEx> &callback,
	registerCallback_sprd_1_2_cb _hidl_cb)
{
	return validateAndCall(
	    this, SupplicantStatusCode::FAILURE_IFACE_INVALID,
	    &StaIface::registerCallbackInternal_sprd_1_2, _hidl_cb, callback);
}
...
SupplicantStatus StaIface::registerCallbackInternal_sprd_1_2(
    const sp<ISupplicantStaIfaceCallbackEx> &callback)
{
    HidlManager *hidl_manager = HidlManager::getInstance();
    if (!hidl_manager ||
	    hidl_manager->addStaIfaceCallbackHidlObjectEx(ifname_, callback)) {
        return {SupplicantStatusCode::FAILURE_UNKNOWN, ""};
    }
    return {SupplicantStatusCode::SUCCESS, ""};
}
...
Return<void> StaIface::setParamToSupplicant(const hidl_string& key, const hidl_string& value,
		setParamToSupplicant_cb _hidl_cb)
{
    return validateAndCall(
        this, SupplicantStatusCode::FAILURE_NETWORK_INVALID,
        &StaIface::setParamToSupplicantInternal, _hidl_cb, key, value);
}

SupplicantStatus StaIface::setParamToSupplicantInternal(const std::string& key, const std::string& value)
{
    struct wpa_supplicant *wpa_s = retrieveIfacePtr();
    char cmdValue[1024];
    ...
    wpa_printf(MSG_DEBUG, "setParamToSupplicantInternal key=%s, value=%s, empty=%d",
           key.c_str(), value.c_str(), value.empty());
#ifdef CONFIG_BGSCAN
    if (!key.compare("SET_BGSCAN")) {
        os_free(wpa_s->conf->bgscan);
        wpa_s->conf->bgscan = os_strdup(cmdValue);
