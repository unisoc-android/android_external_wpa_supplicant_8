...
SupplicantStatus P2pIface::setMacRandomizationInternal(bool enable)
{
	...
	if (enable) {
		...
		if (wpa_drv_set_p2p_mac_addr(wpa_s, NULL) < 0) {
...
Return<void> P2pIface::setParamToP2pSupplicant(const hidl_string& key, const hidl_string& value, setParamToP2pSupplicant_cb _hidl_cb) {
	return validateAndCall(
        this, SupplicantStatusCode::FAILURE_NETWORK_INVALID,
        &P2pIface::setParamToP2pSupplicantInternal, _hidl_cb, key, value);
}

Return<void> P2pIface::getParamToP2pSupplicant(const hidl_string& key, getParamToP2pSupplicant_cb _hidl_cb) {
	return validateAndCall(
        this, SupplicantStatusCode::FAILURE_NETWORK_INVALID,
        &P2pIface::getParamToP2pSupplicantInternal, _hidl_cb, key);
}

SupplicantStatus P2pIface::setParamToP2pSupplicantInternal(const std::string& key, const std::string& value)
{
    struct wpa_supplicant *wpa_s = retrieveIfacePtr();
    char cmdValue[1024];
    memset(cmdValue,0,1024);
    strncpy(cmdValue, value.c_str(), value.size());

    wpa_printf(MSG_DEBUG, "setParamToP2pSupplicantInternal key=%s, value=%s, empty=%d",
           key.c_str(), value.c_str(), value.empty());
    if (!key.compare("SET_P2P_BAND p2p_listen_reg_class")) {
        wpa_s->conf->p2p_listen_reg_class= atoi(cmdValue);
    } else if (!key.compare("SET_P2P_BAND p2p_listen_channel")) {
        wpa_s->conf->p2p_listen_channel= atoi(cmdValue);
    } else if (!key.compare("SET_P2P_BAND p2p_oper_reg_class")) {
        wpa_s->conf->p2p_oper_reg_class= atoi(cmdValue);
    } else if (!key.compare("SET_P2P_BAND p2p_oper_channel")) {
        wpa_s->conf->p2p_oper_channel= atoi(cmdValue);
    }
    return {SupplicantStatusCode::SUCCESS, ""};
}

std::pair<SupplicantStatus, uint32_t> P2pIface::getParamToP2pSupplicantInternal(const std::string& key)
{
    struct wpa_supplicant *wpa_s = retrieveIfacePtr();
    uint32_t value = 0;
    wpa_printf(MSG_DEBUG, "getParamToP2pSupplicantInternal key=%s",
           key.c_str());
    if (!key.compare("GET_P2P_BAND p2p_listen_reg_class")) {
        value = wpa_s->conf->p2p_listen_reg_class;
    } else if (!key.compare("GET_P2P_BAND p2p_listen_channel")) {
        value = wpa_s->conf->p2p_listen_channel;
    } else if (!key.compare("GET_P2P_BAND p2p_oper_reg_class")) {
        value = wpa_s->conf->p2p_oper_reg_class;
    } else if (!key.compare("GET_P2P_BAND p2p_oper_channel")) {
        value = wpa_s->conf->p2p_oper_channel;
    } else {
        return {{SupplicantStatusCode::FAILURE_UNKNOWN, ""}, {}};
    }
    return {{SupplicantStatusCode::SUCCESS, ""}, value};
}
